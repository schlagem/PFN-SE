...
def plot_durations(show_result=False):
    ...
    real_durations_t = torch.tensor(real_durations, dtype=torch.float)
    if show_result:
        plt.title('Result')
    else:
        plt.clf()
    ...
    plt.xlabel('Episode')
    plt.ylabel('Reward')
    plt.plot(durations_t.numpy(), c="tab:blue")
    plt.plot(real_durations_t.numpy(), c="tab:orange")
    # plt.yscale("symlog")
    # Take 100 episode averages and plot them too
    if len(durations_t) >= 10:
        means = durations_t.unfold(0, 10, 1).mean(1).view(-1)
        means = torch.cat((torch.full((10,), -1000), means))
        print("Proxy avg reward: ", means[-1])
        plt.plot(means.numpy(), c="tab:green")

        means = real_durations_t.unfold(0, 10, 1).mean(1).view(-1)
        means = torch.cat((torch.full((10,), -1000), means))
        print("Real avg reward: ", means[-1])
        plt.plot(means.numpy(), c="tab:red")
        print("------------------------------------")
...
if __name__ == '__main__':
    ...
    env = ArtificialEnv() # simple_env.SimpleEnv()
    # env = gym.make("Pendulum-v1")
    real_env = gym.make("Pendulum-v1") # simple_env.SimpleEnv()
    agent = SAC_Agent()
    # policy = PolicyNetwork(3, 1, 0.001)
    # policy.load_state_dict(torch.load("policy_trained_on_REAL_pendulum.pt"))
    # env.update_context(policy)
    ...
    for EP in range(EPISODE):
        state, _ = env.reset()
        score, done, truncated = 0.0, False, False
        ...
        real_sum_reward = 0
        for k in count():
            act, log_prob = agent.choose_action(torch.FloatTensor(state))
            #act, log_prob = policy(torch.FloatTensor(state))
            state, reward, terminated, truncated, _ = real_env.step(act.detach().cpu().numpy())
            real_sum_reward += reward
            if terminated or truncated: #or k > 200:
                #print(k)
                #print("Real reward sum: ", real_sum_reward)
                #print("-----------------")
                real_durations.append(real_sum_reward)
                break
        plot_durations()
    ...
    plt.ioff()
    plt.show()
    real_env = gym.make("Pendulum-v1", render_mode="human") # simple_env.SimpleEnv()
    for t in range(10): # test episodes
        state, _ = real_env.reset()
        score, done, truncated = 0.0, False, False
        while not done and not truncated:
            action, log_prob = agent.choose_action(torch.FloatTensor(state))
            action = action.detach().cpu().numpy()
            state, reward, done, truncated, _ = real_env.step(action)
    # torch.save(agent.PI.state_dict(), "policy_trained_on_REAL_Pendulum_1.pt")
print('Complete')
